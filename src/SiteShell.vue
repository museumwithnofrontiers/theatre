...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('theatre.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('theatre.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('theatre.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
